 ...
 [MvvmCross.Preserve(AllMembers = true)]
        ...
        : MvxIosTask, IMvxPictureChooserTask
    {
            ...
            Action<Stream, string> pictureAvailable, Action assumeCancelled)
        {
            ...
            var presentedViewController = UIApplication.SharedApplication.KeyWindow.RootViewController?.PresentedViewController;
            if (presentedViewController != null)
                presentedViewController.PresentViewController(picker, true, () => { });
            else
            {
                UIApplication.SharedApplication.KeyWindow.RootViewController.PresentViewController(picker, true, () =>
                {
        ...
        {
            if (_currentlyActive)
                Mvx.IoCProvider.Resolve<IMvxLog>().Warn("MvxImagePickerTask called when task already active");
        ...
        {
            if (!_currentlyActive)
                Mvx.IoCProvider.Resolve<IMvxLog>().Warn("Tried to clear currently active - but already cleared");
